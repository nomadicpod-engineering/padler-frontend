...
export default function TripJotterCompanyRoutesPage() {
  ...
  const [sort, setSort] = useState('originAsc');
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [company, list] = await Promise.all([
        ...
        fetchTripJotterCompanyRoutes(companyId, page, TRIP_JOTTER_PAGE_SIZE, q)
      ]);
      ...
      setSort(list.sort ?? 'originAsc');
    } catch (e) {
      if (isForbiddenError(e)) setForbidden(true);
      else setError(e instanceof Error ? e.message : 'Unable to load routes');
    } finally {
      setLoading(false);
    }
  }, [companyId, page, q]);
  ...
  const columns = useMemo<DataTableColumn<RouteRow>[]>(
    () => [
      serialColumn<RouteRow>({ page, pageSize: TRIP_JOTTER_PAGE_SIZE }),
      {
        header: 'Origin → destination',
        id: 'route',
        cell: ({ row }) =>
          String(
            row.original.originTerminalName && row.original.destinationTerminalName
              ? `${row.original.originTerminalName} → ${row.original.destinationTerminalName}`
              : row.original.name ?? row.original.routeName ?? '—'
          )
      },
      {
        header: 'Status',
  ...
  return (
    <PadlerShell>
      <TripJotterCompanyShell companyLabel={label}>
        <h1 className="mb-2 text-xl font-semibold text-slate-950">Routes</h1>
        <p className="mb-4 text-sm text-slate-500">Company routes between terminals (sorted by origin).</p>
        <ToolsListSearch
          value={q}
          onChange={(value) => {
            setQ(value);
        ...
        {!loading && !forbidden ? (
          <>
            <Card className="overflow-hidden p-0">
              <DataTable columns={columns} data={rows} emptyMessage="No routes for this company yet." />
            </Card>
            <TripJotterListPager
              page={page}
